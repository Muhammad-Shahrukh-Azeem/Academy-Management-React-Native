...
const HomeScreen = () => {
  ...
  useEffect(() => {
    const checkAndInitializeFeeRecords = async () => {
      const feeRecordSnapshot = await firestore()
        .collection('feeRecords')
        .where('month', '==', selectedMonth)
        .where('year', '==', selectedYear)
        .get();
  ...
  const initializeFeeRecordsForMonth = async (month, year) => {
    ...
    studentDocs.forEach((studentDoc) => {
      ...
      // Check if studentData has the required properties
      if (!studentData.name || !studentData.totalFee) {
        console.log("Skipping empty studentData:", studentData);
        return;
      }
      ...
      const feeRecordRef = firestore()
        .collection('feeRecords')
        .doc(`${studentDoc.id}_${month}_${year}`);
      
      // Check if packages and subjects are empty
      const emptyValue = '-';
      const subjects = studentData.subjects && studentData.subjects.length ? studentData.subjects : emptyValue;
      const packages = studentData.packages && studentData.packages.length ? studentData.packages : emptyValue;
